...
function gooseInstallHint() {
  return platform === 'darwin'
    ? 'brew install block-goose (macOS) — see https://block.github.io/goose/docs/getting-started/installation'
...
async function probeMcp(name, env) {
  // Best-effort liveness probe; absence of probe support is a warn, not a pass.
  const probe = spawnSync('goose', ['mcp', 'test', name], {
    env: { ...env, ...process.env }, // shell env > .env (Codex review item #8 — consistent with env.mjs.mergeEnv)
    stdio: ['ignore', 'pipe', 'pipe'],
    timeout: 8000,
  })
  if (probe.status === 0) return { state: 'reachable' }
  if (probe.status === null) return { state: 'timeout' }
  const stderr = (probe.stderr?.toString() || '').toLowerCase()
  if (stderr.includes('unknown') || stderr.includes('unrecognized') || stderr.includes('not a goose')) {
    return { state: 'probe_unsupported', stderr: stderr.slice(0, 120) }
  }
  return { state: 'failed', exit: probe.status, stderr: stderr.slice(0, 200) }
}
...
async function checkMcpServers(env) {
  ...
  for (const ext of manifest.expected) {
    ...
    switch (result.state) {
      ...
      case 'ok': {
        // Liveness probe
        const probe = await probeMcp(ext.name, env)
        if (probe.state === 'reachable') {
          pass(`MCP: ${ext.name}`, 'configured + reachable')
        } else if (probe.state === 'timeout') {
          warn(`MCP: ${ext.name}`, 'configured; liveness probe timed out', `Try: goose mcp test ${ext.name}`)
        } else if (probe.state === 'probe_unsupported') {
          warn(`MCP: ${ext.name}`, 'configured; liveness probe unsupported by this goose version', 'Upgrade goose for `goose mcp test` support')
        } else {
          fail(`MCP: ${ext.name}`, `configured but probe failed (exit ${probe.exit})`, `Run: goose mcp test ${ext.name}\n      ${probe.stderr || ''}`)
        }
        break
      }
    }
